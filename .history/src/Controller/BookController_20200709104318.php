...
class BookController extends AbstractController
{
    ...
    public function addToCart(Request $request, int $id) {
        ...
        $session->set('total', ($total == null) ? $book->getPrice() : ($total+ $book->getPrice()));
        if($childBookCount > 5){
            $reduceChildAmount = $session->get('subTotal') * (10/100);
            $session->set('total',  ($session->get('total')+ $reduceChildAmount));
        }
        
        if($totalBookCount > 10){
            $reduceTotalAmount = $session->get('subTotal') * (10/100);
        }
