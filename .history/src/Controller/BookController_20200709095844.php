...
class BookController extends AbstractController
{
    ...
    public function addToCart(Request $request, int $id) {
        ...
        if($book->getCategoryId() == 1) {
            ...
            $session->set('childBookTotal', ($childBookTotal == null) ? 1 : ($childBookTotal + $book->getPrice()));
        }
        $session->set('totalBookCount', ($childBookCount == null) ? 1 : ($childBookCount+ 1));
        $session->set('subTotal', ($childBookCount == null) ? 1 : ($childBookCount+ 1));
        $session->set('discountAmount', ($childBookCount == null) ? 1 : ($childBookCount+ 1));
        $session->set('total', ($childBookCount == null) ? 1 : ($childBookCount+ 1));
