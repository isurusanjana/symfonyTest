...
class BookController extends AbstractController
{
    ...
    public function addToCart(Request $request, int $id) {
        ...
        if ($session->get('bookSelected') != null && array_key_exists($book->getId(),$bookSelected)){
            $selectedBookCount = $bookSelected[$book->getId()][4] + 1;
        } else {
            $selectedBookCount = 1;
        }

        $newBook = [$book->getId(), $book->getName(), $book->getPrice(), $book->getcover_image(),$selectedBookCount];
        ...
        var_dump('Total',$session->get('total'));
        var_dump('totalBookCount',$session->get('totalBookCount'));
        var_dump($session->get('bookSelected'));
